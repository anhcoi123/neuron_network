...
class LogisticLayer():
    ...
    def forward(self, inp):
        ...
        outp = self._fire(inp)
        self.outp = outp
        # print("%dx%d"%(outp.shape[0],outp.shape[1]))
        # print("Output %d"%(np.asarray(outp).shape[0]))
        # print(np.asarray(outp).shape)
        return outp
    def computeDerivativeOutput(self, next_derivatives):
        # print(self.inp.T)
        # print(target-self.outp)
        # self.deltas=-1 * np.dot(self.inp.T,(target - self.outp))
    ...
    def computeDerivative(self, next_derivatives, next_weights):
        ...
        # print("Dado %d"%(np.asarray(dado).shape[0]))
        # print(np.asarray(dado).shape)
        # print("Dado size %dx%d"%(dado.shape[0],dado.shape[1]))
        self.deltas = np.asarray((dado * \
        np.dot(next_derivatives, next_weights)))

        # self.deltas = np.asarray(np.dot((next_derivatives* next_weights).T,dado)).T
